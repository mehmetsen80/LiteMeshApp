...
//
//@Slf4j
//public class RetryFilterStrategy implements  GatewayFilter {
//
...
//
//
//    public RetryFilterStrategy(ApiRoute apiRoute, FilterConfig filter){
...
////
////            if (statusCode != null && statusCode.is5xxServerError()) {
////                // Capture the error response and trigger retry
////                DataBuffer buffer = exchange.getResponse().bufferFactory().wrap(fullBody.getBytes(StandardCharsets.UTF_8));
////                exchange.getResponse().writeWith(Mono.just(buffer));
////                return Mono.error(new RuntimeException(fullBody));  // Trigger retry on 5xx error
////            }
////
////            return Mono.empty();  // No errors, continue normally
////        });
////    }
//
//
...
////
////                .then(Mono.defer(() -> {
////                    // Ensure the BodyCaptureResponse has processed the response
////                    return responseDecorator.getBody().flatMap(body -> {
////                        // Handle the captured response and trigger retry if necessary
////                        return handleResponse(responseDecorator, mutatedExchange);
////                    });
////                }))
//////                .onErrorResume(throwable -> {
//////                    // This handles any errors during the response processing
//////                    log.error("Error in response processing for route: {}", apiRoute.getRouteIdentifier());
//////                    return Mono.error(throwable); // Propagate the error to trigger retry
//////                })
...
//
////
////        // Process the chain with the mutated exchange (includes BodyCaptureRequest and BodyCaptureResponse)
////        // Filter and capture the response body
////        return chain.filter(mutatedExchange)
////                .then(Mono.defer(() -> responseDecorator.getBody().flatMap(fullBody -> {
////                    HttpStatusCode statusCode = responseDecorator.getStatusCode();
////                    log.info("Downstream response status: {}", statusCode);
////
////                    if (statusCode != null && statusCode.is5xxServerError()) {
////                        // Log and return an error to trigger retry
////                        log.info("Captured full body: {}", fullBody);
////                        return Mono.error(new RuntimeException(fullBody));  // Trigger retry on 5xx error
////                    }
////
////                    return Mono.empty();  // No errors, continue normally
////                })))
////                .retryWhen(retryBackoffSpec)  // Retry on errors (including response errors)
////                .onErrorResume(throwable -> {
////                    log.error("Retries failed for route: {}", apiRoute.getRouteIdentifier());
////                    return handleErrorFallback(mutatedExchange, throwable);
////                }).then();
